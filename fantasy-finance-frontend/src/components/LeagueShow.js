...
class LeagueShow extends Component{
  ...
  componentDidMount(){
    ...
    .then(data=>{
      let sortedPortfolios = _.sortBy(data.portfolios,'name')
          ...
          <Table celled>
            <Table.Header>
              <Table.Row onClick={(event)=>this.handleClick(event.target)}>
                <Table.HeaderCell id="ranking">Ranking</Table.HeaderCell>
            ...
            <Table.Body>
              {this.state.portfolios.map(p=><PortfolioRow key={p.name} name={p.name} user={p.username} value={p.value} change={p.total_change}/>)}
            </Table.Body>
            </Table>
        </div>
      )
